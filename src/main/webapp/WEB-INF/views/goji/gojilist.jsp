...
<%@ include file="../include/header.jsp" %>
    ...
    <nav aria-label="Page navigation example">
      ...
      <c:if test="${pageMaker.prev}">
        <li class="page-item">
          <a class="page-link" href="/goji/gojilist${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a>
        </li>
       </c:if>
       <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
        <li class="page-item"><a class="page-link" href="/goji/gojilist${pageMaker.makeSearch(idx)}">${idx}</a></li>
       </c:forEach>
       <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
        <li class="page-item">
          <a class="page-link" href="/goji/gojilist${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a>
        </li>
       </c:if>
      </ul>
    </nav>
...
$(function(){
    $('#searchBtn').click(function() {
      self.location = "/goji/gojilist" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
    });
  });   
</script>
</html>