	                                    ...
	                                    <td>
	                                        <span style='float:right'>
	                                           <button type="submit" id="list_btn" class="btn btn-default">목록</button>
												...
												<a type="button" class="btn btn-default" href="/goji/writer">글쓰기</a>
                                    	    </span>
                                	    </td>
                            	    </tr>
                        	    </thead>
 	   	...
 	   	<%-- 업데이트 --%>
								...
								<c:forEach items="${replyList}" var="replyList" >
									<div class="border-top border-bottom">
										...
										<button type="button" id="deleteReply-${replyList.rno}" class="btn float-end btn-secondary opacity-75 deleteReply" style="margin-left: 10px">삭제</button> 
										
					
										
										<div><p class="fw-bold">작성자 : ${replyList.replyer}</p></div>
										
										<div class="text-break">내용:<p>${replyList.recontent}</p></div>
										
										<div><p class="fw-light">작성날짜 : <fmt:formatDate value="${replyList.replydate}" pattern="yyyy-MM-dd"/></p></div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#delete_btn").click(function(){
			formObj.attr("action", "/goji/delete");
		...
		$("#update_btn").click(function(){
			formObj.attr("action", "/goji/updateView");
		...
		$("#list_btn").click(function(){
			location.href = "/goji/gojilist?page=${scri.page}"
		...
		$(".replyWriteBtn").on("click", function(){
			...
			formObj1.attr("action", "/goji/replyWrite");
			formObj1.submit();
		});
	...
	deleteReplyBtns.forEach( (element) => {	// 모든 댓글 삭제버튼에 대해서
		element.onclick = (event) => {	// 버튼을 클릭하면 실행할 이벤트 함수를 부여한다
			...
			if(answer === true) {	// 확인을 받으면
				location.href = '${cpath}/goji/deleteReply/' + rno;	// 해당 주소로 이동
